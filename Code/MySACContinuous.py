...
agent = SACAgent(env, gamma=0.99, alpha=0.1,
                 batches=5, batch_size=32,
...
def signal_handler(signal,frame):
    print('Ctrl+C to save model')
    agent.save_models("Force_Save3")
    exit(0)
...
def train():
    ...
    for episode in range(episodes):
        ...
        print("action prob{}".format(agent.actor_net.predict(obs[np.newaxis])[0]))
        print("Q1 Value{}".format(agent.q1_net.predict(obs[np.newaxis])[0]))
        ...
        if(SAVE and episode > 0 and episode %10 == 0):
            np.save(agent.savedic + "/logs/epsdReward4-18-2.npy", np.array(episode_rewards))
            #np.save(agent.savedic + "/logs/stepReward4-16.npy", np.array(all_step_rewards))
            np.save(agent.savedic + "/logs/stepNum4-18-2.npy", np.array(step_count_each_ep))


def test():

    agent.resume = True
    agent.resumeDic = "Force_Save2"
