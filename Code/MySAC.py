...
class SACAgent:
    def __init__(self, env,
                 ...
                 network_learning_rate=0.0001, replayer_capacity=10000,
...
def train():
    ...
    for episode in range(episodes):
        ...
        if(SAVE and episode > 0 and episode %10 == 0):
            np.save(agent.savedic + "/logs/epsdReward4-20.npy", np.array(episode_rewards))
            #np.save(agent.savedic + "/logs/stepReward4-16.npy", np.array(all_step_rewards))
            np.save(agent.savedic + "/logs/stepNum4-20.npy", np.array(step_count_each_ep))


def test():

    agent.resume = True
    agent.resumeDic = "ep512#step582674"
    agent.load_models("ep512#step582674")
