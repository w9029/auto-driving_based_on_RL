...
from config import INPUT_DIM, ROI, THROTTLE_REWARD_WEIGHT, MAX_THROTTLE, MIN_THROTTLE, \
    REWARD_CRASH, CRASH_SPEED_WEIGHT, EXPECT_SPEED, FAST_SPEED_WEIGHT, LOW_SPEED_PUNISH, CTE_PUNISH_WEIGHT
...
class DonkeyUnitySimContoller:
    ...
    def __init__(self, level, port=9090, max_cte_error=1.0):
        self.level = level
        self.verbose = False
...
class DonkeyUnitySimHandler(IMesgHandler):
    ...
    def observe(self):
        ...
        #observation = self.original_image
    ...
    def calc_reward(self, done):
        ...
        if done:
            # 结束惩罚，离开轨道时给一个大的惩罚，并且对此时的油门值进行惩罚
        ...
        if self.speed < EXPECT_SPEED:
            # 低速惩罚 地狱期望速度的部分乘以权重
            speed_reward = (self.speed - EXPECT_SPEED) * LOW_SPEED_PUNISH
        else:
            # 高速奖励 高出期望速度的部分乘以权重
            speed_reward = (self.speed - EXPECT_SPEED) * FAST_SPEED_WEIGHT

        # cte惩罚： 0-1的惩罚值
        cte_punish = - math.fabs(self.cte) / self.max_cte_error * CTE_PUNISH_WEIGHT
        return 1 + throttle_reward + speed_reward + cte_punish
